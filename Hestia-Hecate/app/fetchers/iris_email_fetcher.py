...
logger = logging.getLogger("hestia_hecate.iris_email_fetcher")


class IrisEmailFetcher(BaseFetcher):
    ...
    def connect(self) -> bool:
        try:
            ...
            logger.warning(
                "event=iris_fetcher_connect_failed Iris fetcher connect failed: %s", error)
            return False
    ...
    def fetch_new_data(self, since_date: datetime, custom_filter: str = "") -> list[dict[str, Any]]:
        ...
        try:
            response = requests.post(
                f"{self.hub_api_url}/route/iris/api/email/messages",
                json=envelope,
                timeout=12,
            )
            ...
            logger.warning(
                "event=iris_fetcher_request_failed Iris fetcher request failed: %s", error)
            return []
