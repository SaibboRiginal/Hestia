"""Google Calendar fetcher for Hestia-Hecate.
...
logger = logging.getLogger("hestia_hecate.gcal_fetcher")

_TIMEOUT = 20


class GCalFetcher(BaseFetcher):
    """Fetch Google Calendar events via Hub-routed Hecate calls."""
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if resp.status_code < 300:
                return True
            logger.warning(
                "event=gcal_hub_health_check_failed [GCAL] Hub health check failed status=%s", resp.status_code)
        except Exception as exc:
            logger.warning(
                "event=gcal_cannot_reach_hub [GCAL] Cannot reach Hub: %s", exc)
        return False
    ...
    def fetch_new_data(self, since_date: datetime, custom_filter: str = "primary") -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                "event=gcal_list_events_call_failed [GCAL] list_events call failed: %s", exc)
            return []
