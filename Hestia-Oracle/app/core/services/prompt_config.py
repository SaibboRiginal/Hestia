...
_DEFAULT_PROMPTS: dict[str, str] = {
    ...
    "analyst_persona_default": (
        ...
        "FORMATTAZIONE: il client specifica il formato richiesto nelle CLIENT_INSTRUCTIONS. "
        "Se il client chiede HTML, usa tag Telegram validi. "
        "Se il client chiede Markdown, usa solo sintassi Markdown. "
        "Se non specificato, default Markdown: **grassetto**, _corsivo_, [testo](url) per link, • per liste.\n"
        "MAI usare Markdown e HTML mischiati nella stessa risposta.\n\n"
        "STILE FINALE:\n{conversation_style_contract}"
    ),
    "formatter_html_rule": (
        "FORMATTAZIONE OBBLIGATORIA: rispetta il formato richiesto nelle CLIENT_INSTRUCTIONS. "
        "Default Markdown: **testo** per grassetto, _testo_ per corsivo, [testo](url) per link. "
        "Se richiesto HTML: <b>testo</b>, <i>testo</i>, <a href=\"url\">testo</a>. "
        "Per liste usa il simbolo • (bullet) direttamente. "
        "MAI mischiare Markdown e HTML nella stessa risposta."
    ),
    "formatter_alert_template": (
        ...
        "- Non inventare dati.\n\n"
        "{html_format_rule}\n"
        "Per i link, usa SEMPRE il titolo/descrizione dell'elemento come testo del link in formato [testo](url), MAI testi generici.\n"
        "{alert_context_block}"
        "COMMAND: {command}\n"
        "SERVICE_PAYLOAD:\n{payload_text}"
    ),
